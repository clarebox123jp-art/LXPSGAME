...
window.GAME_CHANGELOG = [
  // v5.30.0 — 裝備系統 Phase 2 戰鬥機制(GM 測試期)
  {
    ...
    adminOnly: true,
    brief: [
      '⚔️【裝備真的會打架了!(GM 測試中)】裝備的全部效果正式接上戰鬥引擎:減傷、屬性減免、格擋(成功=傷害減半,最高35%)、迴避、命中、暴擊、普攻/特技/爆發傷害加成、武器詞條命中附加狀態、免疫與狀態抗性(最高50%)、受傷反彈、治療與受療加成、開場增益與護盾、每回合淨化,還有掉寶/知識幣/經驗加成,全部真實生效!',
      '🔨【鐵匠轉職成功!】鐵匠 S1「武器精煉強化」改為提升全隊「攻擊面」裝備效果、S2「防具精煉強化」提升全隊「防禦面」裝備效果(每級+5%,最高+50%,2回合)——鐵匠成為裝備隊的靈魂人物!',
      '🧭【圖鑑新按鈕】裝備欄位最下方新增「推薦裝備」(依英雄定位自動把閒置裝備裝進空格)與「一鍵卸除」(一次脫光光,至寶不受影響),還會顯示減傷/抗性/格擋的目前數值與上限!',
    ],
    items: [
      '★ v5.30.0【裝備 Phase 2 戰鬥引擎】h._gearEff 全面消費:受方 dr(上限50%)/elemDr/allElemDr/critDmgTaken;格擋新機制(與迴避獨立·成功傷害減半·計算最優先·上限35%·貫穿格擋=攻方機率使其失效);迴避/命中併入至寶淨迴避算式;攻方 naDmg/skDmg/burstDmg/allDmg(星鎧護體黃金口徑判別)/crit 併 _critChance;onHit 詞條限普攻與特技(排除爆發/DoT/固定/反彈);反彈機率併至寶反彈點(反彈量沿用 100% 相同傷害前例);免疫/抗性統一守門於 addStatus(合計上限50%·沉默規則⑬=同時參與封技/封普攻/封爆發判定·燃燒涵蓋 hellfire);doHeal 治療/受療加成;戰開 openBuff 11 種+openShield;每回合淨化 cleanse;結算 drop/gold/exp(隊伍取單一英雄合計最大值,比照至寶掉寶口徑)',
      '★ v5.30.0【丙案 BOSS 難度保護(設計表平衡檢查頁老師裁定)】世界龍王與冒險 BOSS(_ZEUS_TRUE_BOSSES)的所有攻擊一律「至少保留原始 40%」傷害下限(爆發維持既有 _wbBurstMinHalf 50%);基準=doDmg 頂部最原始傷害快照,對至寶/技能/裝備所有減傷來源一體適用,防全裝備隊無力化 BOSS',
      '★ v5.30.0【鐵匠 S1/S2 改版(老師甲案)】execSkill/aiUseSkill 雙路徑(鐵律1.128)改掛 _gearBkWeapon/_gearBkArmor buff(+5%/級·上限+50%·2回合);消費點 _gearBkMult 於各裝備效果取值處放大(S1=攻擊面:傷害%/命中/暴擊/onHit機率;S2=防禦面:減傷/屬性減免/格擋/迴避/抗性/淨化/反彈/治療%);免疫類不放大(設計表建議);hero_db.js d/fd+檔尾 sd 雙版同步(鐵律1.232);SKILL_UPGRADE_DEF+圖鑑升級視窗 display case 同步;舊 _blkWeaponBuff/_blkArmorBuff 消費 hooks 保留不再施加(零風險)',
      '★ v5.30.0【推薦裝備/一鍵卸除+三上限提示】_gearAutoEquip 依 HERO_PRIMARY_CLASS 定位評分(稀有度+效果類型加權·含詞條)自動填空槽(不覆蓋玩家手動配置·只用未被任何英雄使用的裝備);_gearUnequipAll 一鍵卸下全部(至寶不受影響);裝備欄位標題下新增 減傷/抗性/格擋 合計與上限狀態行(達上限顯示「已達上限」·設計表補充1)',
    ],
  },
  // v5.29.0 — 裝備圖片第一批(身軀部位 10 張插畫上線·GM 測試期)
  {
    ver: 'v5.29.0',
    date: '2026-08-12',
    adminOnly: true,
    brief: [
      '🖼️【裝備穿新衣!(GM 測試中)】裝備系統的專屬插畫開始上線啦!第一批「身軀部位」10 件全部換上手繪圖:帆布圍裙、消防外套、雨衣、風衣外套、工作背心、園丁工作服、絕緣工作服、羽絨外套、皮夾克、古代鎧甲,每件都有稀有度光暈,打開英雄圖鑑的裝備欄位就看得到!',
      '✨ 還沒畫好的部位會先用可愛的表情符號代替,之後五批(頭部/主手/副手/腿部/飾品)陸續上線,敬請期待!',
    ],
    items: [
      '★ v5.29.0【裝備圖片接線(Phase 5 前置·分批通用)】新增 _gearIconHtml helper:GEAR_DB img 欄位對應 repo 根目錄同名 PNG(_GEAR_IMG_BASE+encodeURIComponent+?v=_GEAR_IMG_VER);裝備列表名稱行 30px 與已裝備槽卡 42px 兩處改讀插畫,圖片未上傳或載入失敗 onerror 自動退回 emoji → 後續批次僅上傳圖檔即生效,零程式改動。部位空槽與選擇器標題維持部位 emoji 不變。',
      '★ v5.29.0【素材版本脫鉤】_GEAR_IMG_VER 獨立於遊戲版號,僅同名圖片重製覆蓋時 bump,避免每次改版全量重抓 60 張圖(共享 iPad 頻寬考量)。',
      '★ v5.29.0【第一批素材】body 槽 10 張(檔名=img 欄位):帆布圍裙/消防外套/雨衣/風衣外套(rare 藍暈)、工作背心/園丁工作服/絕緣工作服(epic 紫暈)、羽絨外套/皮夾克(legend 橘金暈)、古代鎧甲(mythic 桃紫暈),1254×1254 透明背景 PNG。',
    ],
  },
