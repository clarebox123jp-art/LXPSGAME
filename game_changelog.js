...
//  最後更新:2026-07-24  / 目前主程式版本:v4.91.1(主角技能繼承系統:主角覺醒後可繼承 R/SR 英雄的技能與極限爆發·管理員測試)
...
window.GAME_CHANGELOG = [
  ...
  // v4.86.0 — 七修:造型工房三修 + 教學指引兩新章 + 第七章待續入口 + 對白閃過根治 + 回顧模式全內容可選(含未部署的 v4.85.0 全部內容)
  {
    ...
    brief: [
      ...
      '👗【重看劇情：所有內容都回來了，而且由你決定要不要看】以前按「🔁 回顧劇情」，造型工房、名片、認識魔王／英雄升級／商店補給三個教學會被直接跳過。現在這五樣都會跳出小視窗問你「要不要看？」——想複習就按好，只想看故事就按不用，一秒跳過。劇情戰鬥演出、夥伴加入、發劍、主角覺醒本來就照播，所以回顧現在是完整的內容了。',
    ...
    items: [
      ...
      '【素材快取】本版 bump 了 AVATAR_DB_VERSION → 部件圖 URL 帶 ?v=v4.86.0 → SW/瀏覽器/CDN 全數 cache miss 強制重抓(一次性·屬預期)',
      ...
      '【需求6+7・回顧模式】_MS_REVIEW_SKIP_ACTS 全五項(open_avatar_studio / set_card / tutorial_king / tutorial_levelup / tutorial_shop)由靜默略過改走 _msReviewOptionalAct 詢問視窗',
      '→ 文案改表驅動 _MS_REVIEW_OPT_TX(icon/標題/說明/確認鈕 各 premium+cute 雙版·鐵律1.232);2 分鐘沒選自動續播不卡死;首次遊玩(非回顧)完全不觸發此視窗,行為零改變。',
      '→ ★ 稽核結果:battle_ch1_1~ch6_boss 六場劇情戰鬥演出、第一章戰鬥教學卡、join_*/grant_sword_tutorial/awaken_hero/blackout 本就不在略過清單,回顧時照播 → 回顧內容自此 100% 完整。',
      '→ ★ 既有設計保留:第一章戰鬥教學卡若玩家已看過真實戰鬥教學(_tutorialDone / _tutorialMiniDone),只播核心 4 步當複習(v4.81.0 B5),非異常。',
