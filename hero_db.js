...
const HERO_DB = {
  ...
  '阿蘇火山龍王':{hp:60,atk:12,sp:13,spd:15,
    s1:{n:'龍神分身術',c:6,d:'用特技的100%恢復全體友方HP,並使全體友方免疫傷害和不利狀態1回合',fd:'龍王分出無數道炎之分身守護全隊!以特技值的 100% 恢復全體友方的 HP,並使全體友方獲得「無敵」1 回合——期間免疫所有傷害與不利狀態。'},
    s2:{n:'火山噴發',c:4,d:'用(攻擊+速度)的250%對1名目標造成火屬性傷害,並使其陷入「燃燒」狀態2回合',fd:'龍王引動阿蘇火山的怒火,岩漿轟然噴發!以(攻擊值+速度)的 250% 對 1 名對手造成火屬性傷害,並使其陷入「燃燒」狀態 2 回合(行動前後持續損失 HP)。'}
  },
...
const BURST_DB = {
 ...
 '陰陽師':   {n:'四聖降臨',    d:'召喚玄武&白虎(陰陽師HP×150%,5回合)，若朱雀/青龍不存在則補召喚，所有式神HP回滿，特技40%全體攻擊4次，附加2回合減療/緩速/受傷+50%',fd:'召喚玄武&白虎(陰陽師HP×150%/5回合),若朱雀/青龍不存在則補召喚,所有式神HP回滿,特技40%全體×4次,敵全體減療/緩速/受傷+50% 2回合。'},
  ...
  '阿蘇火山龍王':{n:'超光速衝擊波', d:'連段砲擊:每段消耗自己最大HP的10%,以(失去的HP×10+特技100%)轟擊1名目標,打到自己HP低於15%為止;目標倒下自動轉移到HP最高的對手。必中且無視有利,每段獨立機率暴擊', fd:'龍王將自身的生命化為超光速彈藥,展開連段砲擊!每一段消耗自己最大 HP 的 10%,並以(該段失去的 HP × 10 + 特技值 100%)對 1 名目標造成傷害;接著持續用同樣方式消耗 HP 轟擊同一目標,直到自己的 HP 低於 15% 為止。目標若倒下,傷害會自動轉移到下一個 HP 最高的對手。此傷害必中且無視有利狀態,每一段都有獨立機率觸發暴擊。'},
...
const HERO_TRAIT = {
  ...
  '阿蘇火山龍王':{ name:'頑強', icon:'🌋', desc:'自己HP低於10%時(含致命一擊),攻擊力和特技變為2倍,並獲得「不死」狀態2回合(每場戰鬥觸發1次)', fd:'(被動)瀕死的龍王將迸發火山般的怒火——自己的 HP 第一次低於 10% 時(就算是被致命一擊打倒也會以 1 HP 觸發),攻擊力和特技立刻變為 2 倍(持續整場),並獲得「不死」狀態 2 回合(受到致命傷害時保留 1 HP)。每場戰鬥只觸發 1 次。' },
...
const _TRAIT_LV_INFO = {
  ...
  '阿蘇火山龍王':{ base:'HP 低於 10% 時觸發:攻擊/特技×2(整場)+不死 2 回合', bonus:'每升1級 HP 門檻+5%', max:'門檻 30%、不死 3 回合(Lv5)', effect:'(被動)HP 第一次低於門檻時(含致命一擊,以 1 HP 觸發)攻擊與特技翻倍並獲得不死;每場戰鬥限 1 次' },
