// ★ v3.15.33(2026-06-18)— 修正動物學家天賦升級視窗:_TRAIT_LV_INFO 移除重複的「動物學家」定義(舊「寵物效果」殘留,JS 同 key 後者覆蓋前者),改由 v3.15.9「驅除機率 50→90%」正確版生效。｜v3.15.29 新增埃及雙王 BOSS 答題反應對白常數(BOSS_REACT_*_PHARAOH / *_CLEOPATRA)。
...
Object.assign(HERO_TRAIT, {
  '法老王':   { name:'法老威儀', icon:'👑', desc:'回合開始累積「威儀」(傷害+10%/層,最多10層),受傷時可能減1層(天賦越高威儀越穩固);豔后倒下時耗最大HP5%復活她至25%HP',fd:'自己回合開始時獲得1層「威儀」,每層使造成的傷害+10%(最多累積10層);每次受到傷害時有機率減少1層威儀(天賦升級可降低掉層機率、使威儀更穩固,實際機率見「天賦升級視窗」)。當埃及豔后倒下時,法老王會立即耗費自身最大HP的5%,將埃及豔后以最大HP的25%復活(不限次數)──唯有同一時間「同時擊殺」雙王,才能阻止彼此復活。' },
  '埃及豔后': { name:'蛇瞳魅影', icon:'🐍', desc:'回合開始機率使對手減3能量;受傷時機率使攻擊者強力魅惑1回合;法老王倒下時耗最大HP5%復活他至25%HP',fd:'回合開始時有機率使對手隊伍減少3能量。受到傷害時有機率使攻擊者陷入「強力魅惑」1回合。以上兩種機率皆隨天賦升級提升,實際數值見「天賦升級視窗」。當法老王倒下時,埃及豔后會立即耗費自身最大HP的5%,將法老王以最大HP的25%復活(不限次數)──唯有同一時間「同時擊殺」雙王,才能阻止彼此復活。' },
});
// ★★★ 埃及關英雄資料 END ★★★
