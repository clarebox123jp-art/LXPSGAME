...
Object.assign(HERO_DB, {
  ...
  '法老王':{hp:11500,atk:30,sp:44,spd:22,exp:130,
    s1:{n:'王權法杖',c:3,d:'依攻擊+特技150%傷害1名對手並擊退其行動條',fd:'高舉象徵王權的黃金法杖,以攻擊+特技的150%對1名指定對手造成1次光屬性傷害,並將其行動條擊退到最後(本回合最後行動)。'},
    s2:{n:'黃沙風暴',c:7,d:'依攻擊+特技80%對全體造成2次傷害並使其速度-25% 2回合',fd:'召喚漫天黃沙橫掃戰場,以攻擊+特技的80%對全體存活對手造成2次光屬性傷害(每次獨立結算暴擊),並使其速度-25%持續2回合。'}
  },
...
const HERO_TRAIT = {
  ...
  //   護盾基礎 +5/級 規則保留不變
  '神槍手':{ name:'武裝守護', icon:'🛡', desc:'戰鬥開始為全體附加護盾並使全體速度永久提升(隨天賦等級);行動後再補最低 HP 友方護盾',fd:'戰開為全體+30護盾+速度+3(永久,不被消除/奪取)。每次自己行動結束後為最低HP友方補30護盾(取大值覆蓋)。速度加成隨天賦等級提升(Lv1+3 / Lv2+6 / Lv3+9 / Lv4+12 / Lv5+15)。' },
  // ★ v1.0.20260522.7300 — 學生設計英雄系列(5 年 3 班張同學「火柴人」)
  ...
  '風術士':{ name:'風之庇佑', icon:'🍃', desc:'存活時全體友方迴避提升;自己受傷時每回合最多回血1次(無法擋致命傷)', fd:'(被動)風術士存活時,全體友方迴避率提升 +6%~+14%(隨天賦等級)。此外,風術士自己受到傷害時回復最大 HP 的 15%~35%(隨天賦等級),每回合最多觸發 1 次,且無法抵擋會使其倒下的致命傷害。' },
  ...
  '魔界花使‧朱玥':{ name:'百草共鳴', icon:'🌿', desc:'場上每存在 1 名草屬性角色(含自己、敵我雙方),朱玥傷害量 + 治療量 +4%(每升 1 級 +2%/名,Lv5 滿級達 12%/名),上限 +50%',fd:'朱玥與所有花草植物共鳴!場上每存在 1 名草屬性角色(含自己、敵我雙方;需存活),朱玥造成傷害量 + 治療量 +4%。每升 1 級每名加成 +2%(Lv5 滿級達 12%/名,4 名 = 48%、5 名以上一律 50% 封頂)。總加成上限 +50%(不隨等級提升)。' },
  ...
  '小鬼貓與兔':{ name:'惡作劇', icon:'😈', desc:'對手普攻時 25% 機率被干擾(每升 1 級 +5%,Lv5 滿級達 45%):該次傷害打回對手自己(每回合最多 1 次)', fd:'貓與兔的調皮鬼點子總能讓對手自亂陣腳!每當對手要對小鬼貓與兔普通攻擊時,有 25% 機率被干擾:該次傷害直接打回對手身上,小鬼貓與兔毫髮無傷。每升 1 級觸發機率 +5%(Lv5 滿級達 45%)。每回合最多觸發 1 次,避免無限耍對手。' },
};
...
Object.assign(BURST_DB, {
  '法老王':   {n:'太陽神的審判', d:'特技900%光屬性「群體分攤」大傷+復活倒下友方(25%HP)+我方全體恢復HP(恢復量隨爆發升級提升)',fd:'召喚拉神的烈日洪流自天而降,以特技值900%對全體存活對手造成「群體分攤」光屬性巨大傷害(總威力由存活敵人均分,人越多每人受越少);接著拉神的恩典先<b>復活已倒下的友方</b>(以最大HP的25%復活),再使我方全體恢復最大HP的25%(爆發每強化1級恢復量+5%,Lv4達45%)。'},
  '埃及豔后': {n:'尼羅河的詛咒', d:'特技200%全體暗屬性大傷+機率強力查封物品2回合(機率隨爆發升級提升)',fd:'喚醒尼羅河千年詛咒淹沒戰場,以特技值200%對全體存活對手造成暗屬性巨大傷害,並各有50%機率使其陷入「查封物品」狀態2回合(期間無法使用任何道具;爆發每強化1級此機率+5%,Lv4達70%)。'},
});
// 雙 BOSS 天賦(HERO_TRAIT;威儀疊層/互相復活的hook由index.html批二手寫)
Object.assign(HERO_TRAIT, {
  '法老王':   { name:'法老威儀', icon:'👑', desc:'回合開始累積「威儀」(傷害+10%/層,最多10層),受傷時機率減1層(天賦升級↓此機率,越穩);豔后倒下時耗最大HP5%復活她至25%HP',fd:'自己回合開始時獲得1層「威儀」,每層使造成的傷害+10%(最多累積10層);每次受到傷害時有機率減少1層威儀,基礎50%,天賦每升1級此機率-5%(Lv5僅30%,威儀越穩固)。當埃及豔后倒下時,法老王會立即耗費自身最大HP的5%,將埃及豔后以最大HP的25%復活(不限次數)──唯有同一時間「同時擊殺」雙王,才能阻止彼此復活。' },
  '埃及豔后': { name:'蛇瞳魅影', icon:'🐍', desc:'受傷時機率使攻擊者強力魅惑1回合(天賦升級↑此機率);法老王倒下時耗最大HP5%復活他至25%HP',fd:'受到傷害時有機率使攻擊者陷入「強力魅惑」1回合,基礎50%,天賦每升1級此機率+5%(Lv5達70%)。當法老王倒下時,埃及豔后會立即耗費自身最大HP的5%,將法老王以最大HP的25%復活(不限次數)──唯有同一時間「同時擊殺」雙王,才能阻止彼此復活。' },
});
// ★★★ 埃及關英雄資料 END ★★★
