...
(function setupWorldBossModule(){
    ...
    try{
      if(typeof HERO_TRAIT === 'object' && HERO_TRAIT){
        Object.assign(HERO_TRAIT, {
          ...
          '維蘇威火山龍王': { name:'炎之意志', icon:'🐉', desc:'單次受傷上限 1% maxHp;第 4/8/12/16 回合啟動四元素護盾各 1 層(減傷 80%);全隊聯手爆發可無視護盾', fd:'兩千年沉睡淬煉的炎之意志,單次受傷上限為最大 HP 的 1%(即任何一擊最高僅造成 5000 傷害)。每場戰鬥的第 4、8、12、16 回合會自動啟動「四元素護盾」,每個元素各 1 層、共 4 層:所有傷害再減 80%,即使是無視有利狀態的攻擊也無法穿透。需要使用對應屬性(火 / 風 / 土 / 暗)的剋制元素(水 / 土 / 草 / 光)攻擊各 1 次,才能完整破除護盾恢復正常傷害。整場戰鬥共 4 次護盾、16 層,需用心管理破盾節奏。註:當隊伍累積答對 5 / 10 / 15 / 20 題時觸發的「全隊聯手爆發」5000 傷害可以無視護盾直接命中。' },
        });
  ...
  window._wbApplyBossDmgCap = function(boss, rawDmg, opts){
    ...
    //   新版(v3.5.70):護盾改成依回合數啟動(第 4/8/12/16),每元素 1 層
  ...
  //   回合 20 → 系統警告「戰場即將崩毀」
  //   回合 21 → BOSS 強制全員滅絕,結算
  //   ★ v3.5.70 — 玩家完整跑完 20 回合(每回合 1 題),最後一次(第 21 次)答題
  //                行動結束後,輪到 BOSS 時觸發崩毀。
  // ───────────────────────────────────────────────────────────────────
  window._wbConstants = {
    MAX_TURNS: 21,            // ★ v3.5.70:崩毀觸發回合(玩家可完整跑完 1~20 回合)
    BURST_TURN: 10,
    BURST_WARN_TURN: 9,
    COLLAPSE_WARN_TURN: 20,   // ★ v3.5.70:警告改在第 20 回合(原 18)
  ...
  function _wbInstallAdvStage(){
    ...
    ADV.worldboss = {
      ...
      objectives: [
        ...
        '🛡 用 4 種破盾元素削減 BOSS 護盾(第 4/8/12/16 回合啟動)',
        '💥 撐過第 10 回合的「天崩之炎」爆發技',
        '🏆 全員存活擊敗 BOSS 獲得最高排名',
      ],
      rewards: [
  ...
  window._wbAdvBossTurn = function(boss){
    ...
    // ★ v3.5.70(2026-05-24) — 戰場崩毀改為第 21 回合 BOSS turn 觸發
    //   老師決策:讓玩家完整跑完 20 個回合的答題與行動,然後在第 21 回合 BOSS turn
    //             才觸發崩毀全滅。這樣玩家總共會經歷:
    //               開場 1 題 + 第 1~20 回合各 1 題 = 共 21 題答題機會
