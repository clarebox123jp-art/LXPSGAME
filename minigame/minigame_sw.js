...
/* ★ v1.259.0(2026-09-18・老師「荒島中常常遇到音樂和音效卡頓、延遲、突然中斷」)——【SW 端根因兩條】
   ① iOS Safari 播一首 3~5MB 的 BGM 不是抓一次,而是一路發「bytes=N-M」小片段請求(loop 每繞一圈再來一輪);
      舊 rangeFromCache 每接到一個片段請求就 hit.clone().arrayBuffer() 把**整包**從 Cache Storage 讀出來、複製到記憶體、
      再切一小片回去 ⇒ 一首歌播一分鐘要把整個檔案讀十幾次,iPad 上 CPU/記憶體被這件事吃掉 ⇒ 聲音抖、延遲、偶爾直接斷掉。
      改法:同一個 URL 的整包 ArrayBuffer 在 SW 記憶體裡留一份(最多 12 個檔／約 40MB,LRU 淘汰),之後的片段請求直接切,零重讀。
   ② 快取裡沒有的音檔:舊版把帶 Range 的請求原樣丟給網路,回來的是 206,而 206 不能 cache.put ⇒ **音檔永遠進不了快取**
      (除非老師按「完整下載」),每次進場景都重新跟校網要一次,校網一慢就卡。
      改法:快取沒有時,改用**不帶 Range 的乾淨請求**去抓整包(200)存進快取,再從整包切出 Safari 要的那一段回去;
      同一首歌第二次播放起就全走快取,再也不碰網路。只對音訊/影片副檔名這麼做,圖片維持原路徑不變。 */
var _sbufMap = {}, _sbufOrder = [], _SBUF_MAX_N = 12, _SBUF_MAX_BYTES = 40 * 1024 * 1024, _sbufBytes = 0;
function _sbufGet(key, hit){
  if(_sbufMap[key]) return Promise.resolve(_sbufMap[key]);
  return hit.clone().arrayBuffer().then(function(buf){
    try{
      _sbufMap[key] = buf; _sbufOrder.push(key); _sbufBytes += buf.byteLength;
      while((_sbufOrder.length > _SBUF_MAX_N || _sbufBytes > _SBUF_MAX_BYTES) && _sbufOrder.length > 1){
        var k = _sbufOrder.shift(); if(_sbufMap[k]){ _sbufBytes -= _sbufMap[k].byteLength; delete _sbufMap[k]; }
      }
    }catch(e){}
    return buf;
  });
}
function isMediaPath(p){ return /\.(m4a|mp3|aac|ogg|wav|mp4|webm)(\?|$)/i.test(p || ''); }
function rangeFromCache(req, hit){
  ...
  return _sbufGet(req.url, hit).then(function(buf){
...
function cacheFirst(req, bucket, ms){
  var hasRange = !!(req.headers && req.headers.get && req.headers.get('range'));
  /* ★ v1.259.0 音檔快取鍵一律用「不帶 Range 的乾淨 URL」:Cache API 比對不看 Range 標頭,但 put 進去的是整包 200,取時用 req 或 url 都對得到 */
  var media = hasRange && isMediaPath(req.url);
  return caches.match(media ? req.url : req).then(function(hit){
    if(hit) return hasRange ? rangeFromCache(req, hit) : hit;
    if(media){
      /* 快取沒有:用乾淨請求抓整包 → 存快取 → 再切片段回給 Safari(見 rangeFromCache 上方說明②) */
      return Promise.race([ fetch(new Request(req.url, { mode: 'same-origin', credentials: 'same-origin' })), timeoutAfter(ms || 15000) ]).then(function(res){
        if(!(res && res.ok && res.status === 200)) return res;
        var copy = res.clone();
        return caches.open(bucket).then(function(c){ return c.put(req.url, copy); })['catch'](function(){}).then(function(){
          return rangeFromCache(req, res);
        });
...
self.addEventListener('fetch', function(e){
  ...
  /* ══ ★★ v1.266.0(老師「完整下載主程式後,版號沒有同步更新到最新」)★★ ══
     【根因】頁面端的版本檢查 `mgVerFetchLatest()` 是抓本檔(minigame_sw.js?ck=時間戳)、
     用正規式讀 MINI_VERSION 來判斷「線上是不是有新版」。而這支請求以前會掉進本檔**最後那條
     network-first(逾時只有 2.5 秒)**:逾時就退回 caches.match(req),
     可是它的 URL 每次都帶唯一的 ?ck= 時間戳 ⇒ 快取必定 miss ⇒ 回一個 504 ⇒
     頁面端看到 r.ok 是 false ⇒ cb(null) ⇒ **靜默當成「已經是最新版」**。
     一班 25 台同時跟學校 wifi 要這支 70 多 KB 的檔,2.5 秒非常容易超過;
     更關鍵的是**完整下載之後圖片/音樂/資料表全部走快取、只剩這一支還要走網路**,
     等於整台裝置唯一的「對版管道」被一條最短的逾時鍘掉 ⇒ 版號從此停住。
     【修法】本檔自己一律不攔截,交回瀏覽器原生處理(頁面端本來就帶 cache:'no-store',
     一定會去網路問,也不會被任何快取污染)。這支檔案本來就是 SW 腳本本體,
     瀏覽器每次 navigation 都會自己對版,從來不需要也不該由 SW 自己快取。
     ⚠ 只排除這一支檔名,其餘路由一行未動。 */
  if(sameOrigin && url.pathname.indexOf('minigame_sw.js') >= 0) return;
