...
;  /* ★ v1.22.0 回合制戰鬥與武器(全部選配,缺圖自動退回:主角=既有 sprite 幀 + CSS 動作、魔物=既有 island_mon_<k>.png + CSS 動作、武器=emoji):
  ...
  D.BT = {
    ...
    MON_LV_HP: 0.22, MON_LV_ATK: 0.15,        /* 魔物每級成長;★ v1.196.0 老師需求「魔物 HP/能力要隨等級跟主角差不多成長」— 主角 Lv1 HP 在 v1.195.0 從 100→70(−30%)、16 隻寵物 Lv1 HP 也在 v1.191/1.192 全面下修,魔物成長率原本是 v1.166.0 針對「主角 HP100+隊友」抓的平衡點,老師這輪把玩家陣營調弱了,魔物卻沒有跟著調整,兩邊會脫節;由 0.18/0.12 上調到 0.22/0.15(約 +20%/+25%),讓魔物在中後期(等級較高的區域)相對玩家陣營的成長曲線重新對齊。⚠ 這組數字是依比例反推的估計值,不是重新沙盤模擬過的精確解,下一輪有空建議實際跑一次沙盤驗證勝率。 */
