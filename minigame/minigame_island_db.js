...
window.ISL_DB = (function(){
  ...
  D.LAYER_ADJ = {
    /* ★ v1.29.0(2026-09-13・老師看圖:「頭髮還是沒有貼齊，還有錯誤裁切的」)—— 全表改回 1/1/0/0。
       重新以**輪廓線**(alpha>120)逐格量完 8 張髮型 + 8 張服裝 + 2 張基底身體共 18 格×18 張，結論如下:
         ・髮型:少年 h0~h3 的髮頂與基底頭頂差 **0.0 px**(h0 僅 −0.5)、水平中心差 0.0
           ⇒ 四張全部 sx=sy=1、dx=dy=0，一點都不要動。
           少女 h0~h3 的髮頂一律比基底頭頂低 **+4 px(128 格)= +2 px(64 格)**，四張完全一致
           ⇒ 四張都只需 dy = −2，尺寸不變。舊值(0.886/0.928/0.913 配 dy −9.1/−6.7/−7.5)把少女髮型
           縮小 7~11% 又往上推四倍，髮型浮在頭頂上方 ⇒ 這就是老師看到的「沒貼齊」。
         ・服裝:八張的**下擺(鞋底線)與基底身體差 0.0 px**，表示這些衣服本來就是照同一副身體畫的。
           舊值把 c1~c3 縮到 0.62~0.82 是誤判:當初用「衣服寬度應該 ≈ 身體寬度」擬合，
           但背心/草裙/救生衣本來就比赤裸的身體寬(量到 1.15~1.40 倍)，縮了反而變成小一號的衣服浮在身上。
       ⚠ 這張表現在只是「預設值」：GM 在造型工房微調的值存在 localStorage.isl_lyadj，會覆盖本表。 */
    ...
    hair_girl_h0:  { sx:1.000, sy:1.000, dx:0, dy:-2 },
    hair_girl_h1:  { sx:1.000, sy:1.000, dx:0, dy:-2 },
    hair_girl_h2:  { sx:1.000, sy:1.000, dx:0, dy:-2 },
    hair_girl_h3:  { sx:1.000, sy:1.000, dx:0, dy:-2 }
  ...
  D.SCENE.river = {
    ...
    spawn:{
      fish:  { n:3, minGap:3, act:'fish', node:'node_fish', e:'🐟', gives:'fish', label:'石𩼣魚群',
               pool:[{x:13,y:2},{x:15,y:2},{x:12,y:7},{x:15,y:10},{x:23,y:7},{x:26,y:8},{x:23,y:12},{x:28,y:13}] },   /* ★ v1.29.0 原本 8 個裡有 7 個座標落在水域 '~'(v1.115.0 依場景圖重建遮罩後沒重跑稽核),小人走不到 ⇒ 全部移到緊鄰水邊的可走岸格 */
      reed:  { n:3, minGap:2, act:'gather', node:'node_reed', e:'🎋', gives:'reed', label:'蘆葦叢',
               pool:[{x:18,y:3},{x:14,y:10},{x:5,y:9},{x:27,y:8},{x:12,y:18},{x:22,y:19}] },   /* ★ v1.29.0 第 2、4 個原本在水域 '~' */
  ...
  D.SCENE.rock = {
    ...
    spawn:{
      ...
      trash: { n:5, minGap:2, act:'trash', node:'node_trash', e:'🛍', gives:'trash', label:'海廢堆',
               pool:[{x:3,y:12},{x:10,y:14},{x:14,y:13},{x:19,y:12},{x:22,y:13},{x:28,y:13},{x:7,y:13},{x:25,y:11},{x:12,y:12},{x:17,y:12}] },   /* ★ v1.29.0 第 1、7、8 個原本在水域 '~'(岩岸下半是海) */
  ...
  D.SCENE.lake = {
    ...
    spawn:{
      ...
      lotus:  { n:3, minGap:2, act:'gather', node:'node_lotus', e:'🪷', gives:'seed', label:'台灣萍蓬草',
               pool:[{x:11,y:5},{x:20,y:5},{x:11,y:16},{x:21,y:17},{x:6,y:9},{x:26,y:15}] },   /* ★ v1.29.0 第 4 個原本在水域 '~' */
      reed:   { n:2, minGap:2, act:'gather', node:'node_reed', e:'🎋', gives:'reed', label:'蘆葦叢',
               pool:[{x:4,y:5},{x:28,y:4},{x:4,y:19},{x:28,y:19}] },
      pebble: { n:2, minGap:2, act:'gather', node:'node_pebble', e:'⚪', gives:'pebble', label:'卵石灘',
               pool:[{x:14,y:3},{x:18,y:20},{x:3,y:9},{x:29,y:15}] }   /* ★ v1.29.0 第 2 個原本在水域 '~' */
    },
  ...
  D.SCENE.cave = {
    ...
    spawn:{
      ...
      glow:   { n:2, minGap:2, act:'gather', node:'node_glowshroom', e:'🍄', gives:'mushroom', label:'螢光蕈',
               pool:[{x:12,y:10},{x:16,y:12},{x:25,y:14},{x:7,y:11},{x:21,y:5}] }   /* ★ v1.29.0 第 4 個原本在洞窟水潭 '~' */
    },
  ...
  D.SCENE.valley = {   /* ★ v1.109.0 重畫遮罩:溪水改真的不可走,中間留兩座石橋(x=8/22)過河,入口/出口移到南岸乾地 */
    ...
    spawn:{
      ...
      fish:  { n:2, minGap:3, act:'fish', node:'node_fish', e:'🐟', gives:'fish', label:'台灣石𩼣', pool:[{x:6,y:10},{x:20,y:10},{x:26,y:10},{x:12,y:13}] },   /* ★ v1.29.0 第 1、3、4 個原本在溪水 '~' */
