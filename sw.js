  ...
  event.waitUntil(
    caches.keys().then(function(keys){
      return Promise.all(keys.map(function(key){
        ...
        // ★★ v5.140.0(2026-09-03)— 放過「小英雄小遊戲」獨立小程式的快取 ★★
        //   CacheStorage 是整個 origin 共用的,而上面這條判斷會把「不是自己那兩個名字」
        //   的快取全部刪掉 ⇒ 主程式每更新一次版本,minigame/ 的離線快取就會被清光,
        //   舊平板/慢網的學生下次開小遊戲又得整包重抓,完全違背小遊戲的存在目的。
        //   ⇒ 一律保留 'lxps-mini-' 前綴;小遊戲自己的 sw.js 也只清自己的前綴,雙向互不侵犯。
        if(key.indexOf('lxps-mini-') === 0) return;
        /* ★ v3.8.4 — 也放過 sw-light 的舊素材家 'lxpsgame-light-*'：sw-light v3.12.0 起改與本檔共用 ASSET_CACHE，
           舊家由 sw-light activate 一次性搬進 ASSET_CACHE 後自己刪；本檔若先接管就砍掉，等於把那台裝置累積的素材整包清空
           （老師回報「重開遊戲後檔案又被刪除、要重新慢慢抓」的成因之一）。素材只能被搬、不能被砍。 */
        if(key.indexOf('lxpsgame-light-') === 0) return;
      ...
      // ★ v3.8.1 — 清掉續傳佇列裡任何殘留的舊工作紀錄(見 SW_VERSION 註解根因)，
      //   純環境清理，不會觸發任何下載動作。
      return _lxpsIdbDelete('job').catch(function(){});
    })
  );
});
    ...
    if(_lxpsPrecacheBusy){
      try{
        if(client) client.postMessage({
          ...
          batchId: _lxpsPrecacheBatchId,
          done: _lxpsPrecacheDone,
          failed: _lxpsPrecacheFailed,
          total: _lxpsPrecacheTotal,
          resumed: true
        });
      }catch(_eBusy){}
      console.log('[SW v3.8.2] 已有下載工作進行中(batch ' + _lxpsPrecacheBatchId + ')，忽略這次重複的 PRECACHE_URLS，不開第二份');
      return;
    }
    _lxpsPrecacheBusy = true;
...
/* ★★ v3.8.2 — 下載工作防重入狀態（單一真相）★★
   _lxpsPrecacheBusy 為 true 代表「目前已經有一份下載工作在跑」，message 端據此忽略重複的
   PRECACHE_URLS，避免兩份下載互搶頻寬與 Cache Storage（見 message 端註解）。
   其餘三個變數是給「忽略重複請求時回報目前進度」用的即時快照，由 sendProgress() 同步更新，
   讓重送的頁面能立刻接上目前進度，而不是看到一片空白以為沒反應。 */
var _lxpsPrecacheBusy = false;
var _lxpsPrecacheBatchId = '';
var _lxpsPrecacheDone = 0;
var _lxpsPrecacheFailed = 0;
var _lxpsPrecacheTotal = 0;
...
function precacheUrlsInBatches(urls, client, batchId, wantsWebp, conc){
  ...
  function _saveJob(force){
    ...
    _lastSaveAt = now;
    _lxpsIdbPut('job', {
      urls: urls, wantsWebp: wantsWebp, conc: CONC, batchId: batchId,   // ★ v3.8.0 — 存當下自動調速後的 CONC(非固定起點值),續傳才會接著上次調到的速度跑
  ...
  function sendProgress(){
    /* ★ v3.8.2 — 同步更新防重入快照，讓「重複請求被忽略」時能立刻回報真實進度 */
    _lxpsPrecacheBatchId = batchId; _lxpsPrecacheDone = done;
    _lxpsPrecacheFailed = failed; _lxpsPrecacheTotal = total;
    var msg = {
      type: 'PRECACHE_PROGRESS',
      batchId: batchId,
      done: done,
  ...
  return caches.open(ASSET_CACHE).then(function(cache){
    ...
    return scanNext().then(function(){
      ...
      // ★ v3.8.0(2026-09-11・老師需求「有辦法加快主程式完整下載安裝的速度嗎」，裁定乙案)
      //   ─────────────────────────────────────────────────────────────────
      //   舊碼：並行數 CONC 整趟下載固定不變（桌機 8／iPad 3），是「猜」出來的保守值，
      //   猜錯兩種方向都不好——猜低了，網路和裝置明明撐得住卻白白等；猜高了，遇到弱網或
      //   舊機記憶體撐不住反而下載失敗/當機。而且蘋果不開放讀取精確 iPad 型號，光憑 UA
      //   猜不出裝置等級。
      //   改法(比照 TCP 壅塞控制的 AIMD「加法增、乘法減」精神，不靠猜型號，靠**實測**)：
      //   每批下載完，看這批「花多久、有沒有失敗」來決定下一批要不要調並行數——
      //     ①這批零失敗 + 平均每檔耗時夠快(<800ms) ⇒ 判斷網路/裝置還有餘裕，並行數 +1。
      //     ②這批有任何真實失敗、或平均每檔耗時過慢(>3000ms) ⇒ 判斷已經撐不住，
      //       並行數立刻砍掉一半(乘法減，比加法增猛得多，避免"already 卡住還慢慢降"拖更久)。
      //     ③介於中間 ⇒ 維持不變，不需要每批都在調整。
      //   上下界仍以client 提示的裝置等級(conc 參數，iOS-like 給3／桌機給8)當起點去算，
      //   不是無上限亂長：地板=起點的一半(至少2)，天花板=起點的2倍(iOS-like 最高封頂6，
      //   避免萬一網路好到誤判成"桌機等級"還硬衝爆舊 iPad 記憶體)。
      //   目前 CONC 值同步寫進續傳佇列(_saveJob)，SW 重啟續傳時從上次調到的值接著跑，
      //   不會每次都打回起點重新摸索。
      var _concFloor = Math.max(2, Math.floor(CONC / 2));
      var _concCeil  = (CONC <= 3) ? 6 : (CONC * 2);
      var _concIsIosLike = (CONC <= 3);   // 起點就是保守值 ⇒ 視為 iOS-like 裝置,天花板加倍保護
      ...
      function processBatch(){
        ...
        var batch = toFetch.splice(0, CONC);   // ★ v3.5.98 — 原為寫死的 CONCURRENT；v3.8.0 起 CONC 會逐批自動調整
        var _batchStart = Date.now();
        var _batchFailBefore = failed;
        return Promise.all(batch.map(function(pl){
          ...
          // ★ v3.8.0 — AIMD 自動調速:依這批「耗時／有無失敗」決定下一批並行數
          var _batchElapsed = Date.now() - _batchStart;
          var _batchFailCount = failed - _batchFailBefore;
          var _avgPerFile = batch.length > 0 ? (_batchElapsed / batch.length) : 0;
          if(_batchFailCount > 0 || _avgPerFile > 3000){
            // 乘法減:立刻砍半,比加法增猛,避免"已經撐不住還慢慢降"拖更久
            CONC = Math.max(_concFloor, Math.floor(CONC / 2));
          } else if(_avgPerFile < 800 && CONC < _concCeil){
            // 加法增:一次只加 1,穩穩試探還有沒有餘裕
            CONC = CONC + 1;
          }
          sendProgress();
          // iPad 間隔: 每批之間休 100ms 喘口氣
          // ★ v3.5.98 — 改跟著實際並行數走(原讀 IS_IOS_LIKE,真 Mac 桌機會被誤判成 iPad 每批多休 100ms)
          // ★ v3.8.0 — 改吃「當初是否 iOS-like 起點」而非當下 CONC(調高後也該維持 iPad 的喘息間隔,不是只看數字大小)
          return new Promise(function(r){ setTimeout(r, _concIsIosLike ? 150 : 50); });
        }).then(processBatch);
      }
